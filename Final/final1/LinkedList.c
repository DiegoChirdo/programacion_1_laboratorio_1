...
int ll_map(LinkedList* this,int (*pFunc)(void*))
{
    ...
    int len;
    int i;
    LinkedList* partida = ll_newLinkedList();
    Partida* partidaAux;
    if(this != NULL && pFunc != NULL)
    {
        len = ll_len(this);
        for(i=0;i<len;i++)
        {
            if(pFunc(ll_get(this,i))==1)
            {
                partidaAux = (Partida*)ll_get(this,i);
                partida_getDuracion(partidaAux,&duracion);
                partida_getTipo(partidaAux,&tipo);
                if(tipo == 0)
                {
                    costo = duracion * 2;
                }
                ...
                partida_setCosto(partidaAux,costo);
                ll_add(partida,partidaAux);
            }
        }
        if(pFunc(ll_get(partida,0))==1)
        {
            partidaAux = (Partida*)ll_get(partida,0);
            partida_getTipo(partidaAux,&tipo);
            if(tipo == 0)
            {
                controller_savePartidas("pingpong.txt",partida);
            }
            if(tipo == 1)
            {
                controller_savePartidas("pool.txt",partida);
            }
            if(tipo == 2)
            {
                controller_savePartidas("metegol.txt",partida);
            }
            if(tipo == 3)
            {
                controller_savePartidas("dardos.txt",partida);
            }
        }
        retorno = 0;
    }
...
LinkedList* ll_filter(LinkedList* this,int (*fn)(void*))
{
    ...
    if(this != NULL && fn != NULL)
    {
        ...
        for(i=0;i<len;i++)
        {
            Partida* aux = (Partida*)ll_get(this,i);
            if(fn(aux)==1)
            {
               unaPartida = (Partida*)ll_get(this,i);
               partida_getCantidad(unaPartida,&cantidad);
